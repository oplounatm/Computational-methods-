...
p = zeros(N_t+1,1);
start_index = 10*24/dt +1, ; % 10 days = 6*24 h, div. by dt=0.1 gives intervals
stop_index = 200*24/dt +1, ; %200 days
p(start_index:stop_index) = 0.001;
...
for n = 1:N_t
    ...
    loss = floor(V(n+1) + S(n+1) + R(n+1) + I(n+1)) - floor(V(1) + S(1) + R(1) + I(1));
    if loss > 0
        fprintf('loss: %d', loss);
    end
end
